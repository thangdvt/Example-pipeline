...
*/
def project = "FHO.PID.Android-Windows-COV-Example"
    ...
    stages {
        ...
        */
        stage('Build & Sonar') {
                    steps {
                      ...
                      bat "type C:\\Jenkins\\workspace\\FHO.PID.DevOps_HelpDesk\\ANHBT14\\FHO.PID.Android-Windows-COV-Example\\build.gradle"
        ...
        */
        stage('Scan Coverity'){
            steps{
                script{
                    bat "cov-configure.exe --java"
                    bat "cov-build.exe --dir covoutput --instrument  --fs-capture-search . gradlew assembleDebug cleanBuildCache  --no-daemon" // tùy vào dự án sử dụng "build" hay sử dụng "assembleDebug"
                    bat "cov-analyze.exe --dir covoutput  ${analysis}"
                }
            }
        }
        ...
        stage('Commit Coverity'){
            steps{
                script{
                    retry (3) {
                        bat "cov-commit-defects --dir covoutput ${cov_server} --auth-key-file ${cov_key} --stream ${project}"
                    }
                }
            }
        }
