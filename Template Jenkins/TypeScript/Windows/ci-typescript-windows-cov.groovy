...
*/
def project = 'FHO.PID.TypeScript-Windows-Example'
...
pipeline {
    ...
    * NODEJS_HOME = "${tool 'nodejs-12-windows-with-typescript'}": Path to Nodejs
    ...
    */
    environment {
        ...
        NODEJS_HOME = "${tool 'nodejs-12-windows-with-typescript'}"
        PATH = "${env.JAVA_HOME}/bin;${env.SONAR_HOME}/bin;${env.COVERITY_HOME}/bin;${env.BD_HOME}/bin;${env.NODEJS_HOME};${env.PATH}"
    }
    ...
    stages{
        ...
        */
        stage('Scan Coverity') {
            steps{
                bat "cov-configure --typescript -c config.xml"
                bat "cov-build.exe --dir covoutput -c config.xml --fs-capture-search . --no-command"
